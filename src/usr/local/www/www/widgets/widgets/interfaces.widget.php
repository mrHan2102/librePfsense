...
foreach ($ifdescrs as $ifdescr => $ifname):
	...
	if ($ifinfo['pppoelink'] || $ifinfo['pptplink'] || $ifinfo['l2tplink']) {
		...
		// $typeicon = 'sitemap' thẻ cũ;
		$typeicon = './WAN-network.svg';
	}
...
?>
	<tr class = "content-row">
		<td class="type-network" title="<?=htmlspecialchars($ifinfo['if'])?> (<?=htmlspecialchars($ifinfo['macaddr'])?>)">
			<!-- <i class="fa fa-<?=$typeicon?>"></i> thẻ cũ -->
			<img src="<?=$typeicon?>" alt="">
			<!-- <a href="/interfaces.php?if=<?=$ifdescr?>">
				<?=htmlspecialchars($ifname);?>
			</a> -->
		</td>
		<td class = "ip-network" <?=($ifinfo['dhcplink'] ? ' title="via dhcp"':'')?>>
