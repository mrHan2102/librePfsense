...
function get_stats($sitems = array()) {
	...
	$stats['cpu_unit'] = (!in_array('cpu_usage', $sitems)) ? get_cpu_ticks() : '';
	$stats['cpu_total'] = (!in_array('cpu_usage', $sitems)) ? get_cpu_ticks_total() : '';
...
function get_cpu_ticks() {
	$diff = array('user', 'nice', 'sys', 'intr', 'idle');
    $cpuTicks = array_combine($diff, explode(" ", get_single_sysctl('kern.cp_time')));
    $cpuTicksSum = array_sum($cpuTicks) - $cpuTicks['idle'];
    $cpuUsageUnit = ceil(($cpuTicksSum / array_sum($cpuTicks)) * 100);
    return $cpuUsageUnit;
}

function get_cpu_ticks_total() {
	$diff = array('user', 'nice', 'sys', 'intr', 'idle');
    $cpuTicks = array_combine($diff, explode(" ", get_single_sysctl('kern.cp_time')));
    $cpuTicksSum = array_sum($cpuTicks) - $cpuTicks['idle'];
    return $cpuTicksSum;
