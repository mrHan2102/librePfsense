...
if ($_REQUEST && $_REQUEST['ajax']) {
	...
	$data = new stdClass();
	$data->overview = "<tr class='content-row'>";
	...
	foreach ($cmap as $k => $tunnel) {
		...
		$data->tunnel .= "<tr class='content-row'>";
	...
	if (is_array($mobile['pool'])) {
			...
			if(is_array($pool['lease']) && !empty($pool['lease'])){
				foreach ($pool['lease'] as $muser) {
					...
					if ($muser['status'] == 'online') {
						...
						$data->mobile .= "<tr class='content-row'>";
					}
		...
		if ($mucount == 0) {
			$data->mobile .= '<tr class="content-row"><td colspan="3">' . gettext("No mobile leases") . '</tr>';
		}
	} else {
		$data->mobile .= '<tr class="content-row"><td colspan="3">' . gettext("No mobile pools configured") . '</tr>';
	}
...
<div id="<?=htmlspecialchars($widgetkey_nodash)?>-Overview" style="display:block;"  class="table-responsive">
	<table class="table ">
		...
		<tbody class="tbody-props">
			<tr class="content-row"><td colspan="5"><?= htmlspecialchars(gettext("Retrieving overview data")) ?> <i class="fa fa-cog fa-spin"></i></td></tr>
		</tbody>
	</table>
</div>
<div class="table-responsive" id="<?=htmlspecialchars($widgetkey_nodash)?>-tunnel" style="display:none;">
	...
	<tbody class="tbody-props">
		<tr class="content-row"><td colspan="4"><?= htmlspecialchars(gettext("Retrieving tunnel data"))?> <i class="fa fa-cog fa-spin"></i></td></tr>
	</tbody>
	</table>
</div>
...
<?php if (is_array($mobile['pool'])): ?>
		...
		<tbody class="tbody-props">
			<tr class="content-row"><td colspan="3"><?= htmlspecialchars(gettext("Retrieving mobile data")) ?> <i class="fa fa-cog fa-spin"></i></td></tr>
		</tbody>
<?php else:?>
		<thead class="thead-props">
			<tr class="title-row">
