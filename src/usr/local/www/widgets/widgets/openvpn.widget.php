...
if (!function_exists('printPanel')) {
	function printPanel($widgetkey) {
		...
		$opstring .=		"<table class=\"table sortable-theme-bootstrap\" data-sortable>";
		$opstring .=			"<thead class='thead-props'>";
		$opstring .=				"<tr class='title-row'>";
		$opstring .=					"<th>" . gettext('Name/Time') . "</th>";
		$opstring .=					"<th>" . gettext('Real/Virtual IP') . "</th>";
		$opstring .=					"<th></th>";
		$opstring .=				"</tr>";
		$opstring .=			"</thead>";
		$opstring .=			"<tbody class='tbody-props'>";
		...
		$opstring .=				"<tr class='content-row' name=\"" . "r:" . $server['mgmt'] . ":" . $conn['remote_host'] . "\" class=\"" . $evenRowClass . "\">";
		$opstring .=					"<td>";
		...
		$opstring .=				"<tr class='content-row' name=\"r:" . $sk_server['port'] . ":" . $sk_server['remote_host'] . "\">";
		$opstring .=					"<td>";
		...
		$opstring .=				"<tr class='content-row' name=\"r:" . $client['port'] . ":" . $client['remote_host'] . "\">";
		$opstring .=					"<td>";
		...
		if (strlen($none_to_display_text) > 0) {
			print('<table class="table"><tbody class="tbody-props"><td class="text-center">' . $none_to_display_text . '</td></tbody></table>');
		}
	}
}
