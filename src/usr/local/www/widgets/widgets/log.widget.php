...
?>
		<tr class = "content-row">
			...
			role="button" data-toggle="popover" data-trigger="hover"
				...
				data-content="<?=htmlspecialchars($rule)?>"> 
				<!-- update icon act -->
				<img src="<?=$iconfn?>" alt=""> 
			</a></td>
			<td title="<?=htmlspecialchars($filterent['time'])?>"><?=substr(htmlspecialchars($filterent['time']),0,-3)?></td>
			<!-- add class -->
			<td class="type-network"><img src="<?php 
					if($filterent['interface'] == 'WAN') {
						echo './WAN-network.svg';
					} elseif($filterent['interface'] == 'LAN') {
						echo './LAN-network.svg';
					}
					?>" alt=""> &nbsp; <?=htmlspecialchars($filterent['interface']);?>
			</td>
