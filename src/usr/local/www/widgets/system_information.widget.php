	...
	<div class = "container-system-information">
		<div class = "system-charts">
			<div class="chart">
				...
				<div class="chart-content"><?=$memUsage?>% <br> of <?= sprintf("%.0f", get_single_sysctl('hw.physmem') / (1024*1024)) ?> MiB</div>
			</div>
			<div class="chart">
				<canvas id="cpu" class="chart-info"></canvas>
				<div class="chart-content">10% <br> of 10GB</div>
			</div>
			<div class="chart">
				<canvas id="swap" class="chart-info"></canvas>
				<?php $swapUsage = swap_usage(); ?>
				<div class="chart-content"><?=$swapUsage?>% <br> of <?= sprintf("%.0f", `/usr/sbin/swapinfo -m | /usr/bin/tail -1 | /usr/bin/awk '{ print $2;}'`) ?> MiB</div>
			</div>
		</div>
		...
		<div class = "system-charts">
			<div class = "chart">
				...
				<div class = "chart-content-2">
					<?=$pfstateusage?>% <br> 
					<span id="pfstate"><?= htmlspecialchars($pfstatetext)?></span><br>
			...
			<div class="chart">
				...
				<div class="chart-content-2"><?=$mbufusage?>% <br> <span id="mbuf"><?= htmlspecialchars($mbufstext)?></div>
			</div>
		</div>
		<div class = "check-status">
			<div class = "status">
				<?php $pti = get_single_sysctl('vm.pmap.pti');
					...
					$rows_displayed = true; ?>
					<input type="checkbox" name = "kernel" <?=($pti == 0) ? 'checked' : '';?>>
			...
			<div class = "status">
				<?php $mds = get_single_sysctl('hw.mds_disable_state');
					...
					$rows_displayed = true; ?>
					<input type="checkbox" name = "mds" <?=(ucwords(htmlspecialchars($mds)) == 'Inactive') ? '' : 'checked';?>>
			...
			<div class = "status">
				<?php if (!in_array('hwcrypto', $skipsysinfoitems)):
					$rows_displayed = true; ?>
					<?php if ($hwcrypto): ?>
					<input type="checkbox" name = "kernel" <?=(htmlspecialchars(crypto_accel_get_algs($hwcrypto)) == '') ? '' : 'checked';?>>
...
<?php get_mbuf($mbuf_stext, $mbuf_usage); ?>
	...
	var chartCPU = drawChart(id = 'cpu', text = 'CPU', using = 10, free = 90, fontSize = '15px', color = '#209AFF')
